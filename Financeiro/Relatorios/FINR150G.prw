...
*/
User Function FINR150G()

Local oReport  
Private OFONT12N := TFont():New("Times New Roman",11,11,.T.,.F.,5,.T.,5,.T.,.F.)
PRIVATE oFont11N := TFont():New("Times New Roman",10,10,.T.,.T.,5,.T.,5,.T.,.F.) 
PRIVATE oFont17N := TFont():New("Times New Roman",17,17,.T.,.T.,5,.T.,5,.T.,.F.)
PRIVATE oFont14N := TFont():New("Times New Roman",14,14,.T.,.T.,5,.T.,5,.T.,.F.)
...
oReport:SetPageFooter(6,{|| ImpRoda(oReport)})  // define rodape'
/*
GESTAO - inicio */
oReport:SetUseGC(.F.)
/* GESTAO - fim
...
//³  Secao 1  ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÙ
oSection1 := TRSection():New(oReport,STR0061,{"SE2","SA2","SED"},aOrdem)
...
TRCell():New(oSection1,"ED_CONTA"	,"SED","Conta Contabil"				,,TamSX3("ED_CONTA")[1] + 5,.F.,)  				
...
TRCell():New(oSection1,"VAL_NOMI"	,	  ,"Descontos"	,cPictTit,nTamVal+3,.F.,) //"Tit Vencidos" + "Valor Nominal"
...
TRCell():New(oSection1,"E2_HIST"	,"SE2",IIf(cPaisloc =="MEX",STR0063,STR0059) ,,,.F.)  			//"Historico(Vencidos+Vencer)"
...
oSection1:Cell("ED_CONTA"	):SetBlock( { || POSICIONE("SED",1,XFILIAL("SED")+SE2->E2_NATUREZ,"ED_CONTA") })
...
//oSection1:Cell("E2_HIST"):SetAutoWidth(80)
oSection1:Cell("E2_HIST"):SetLineBreak(.T.)		//Quebra de linha automatica
...
For nInc := 1 To Len( aSM0 )
	...
	If !Empty(cFilAte) .AND. aSM0[nInc][1] == cEmpAnt .AND. (aSM0[nInc][2] >= cFilDe .and. aSM0[nInc][2] <= cFilAte)
		...
		#IFDEF TOP
			if TcSrvType() != "AS/400"
				cQuery += " AND SE2.E2_NUM     BETWEEN '"+ mv_par01+ "' AND '"+ mv_par02 + "'"
				cQuery += " AND SE2.E2_PREFIXO BETWEEN '"+ mv_par03+ "' AND '"+ mv_par04 + "'"
				cQuery += " AND (SE2.E2_MULTNAT = '1' OR (SE2.E2_NATUREZ BETWEEN '"+MV_PAR05+"' AND '"+MV_PAR06+"'))"
				cQuery += " AND SE2.E2_VENCREA BETWEEN '"+ DTOS(mv_par07)+ "' AND '"+ DTOS(mv_par08) + "'"
				cQuery += " AND SE2.E2_PORTADO BETWEEN '"+ mv_par09+ "' AND '"+ mv_par10 + "'"
				cQuery += " AND SE2.E2_FORNECE BETWEEN '"+ mv_par11+ "' AND '"+ mv_par12 + "'"
				cQuery += " AND SE2.E2_EMISSAO BETWEEN '"+ DTOS(mv_par13)+ "' AND '"+ DTOS(mv_par14) + "'"
				cQuery += " AND SE2.E2_LOJA    BETWEEN '"+ mv_par25 + "' AND '"+ mv_par26 + "'"
				...
				If !Empty(mv_par30) // Deseja imprimir apenas os tipos do parametro 30
					cQuery += " AND SE2.E2_TIPO IN "+FormatIn(mv_par30,";") 
				ElseIf !Empty(Mv_par31) // Deseja excluir os tipos do parametro 31
					cQuery += " AND SE2.E2_TIPO NOT IN "+FormatIn(mv_par31,";")
				EndIf
				If mv_par32 == 1
				...
				If mv_par27 == 2
					...
					cQuery += " AND SE2.E2_TIPO NOT IN "+FormatIn(MV_CPNEG,";")			
				Endif		
				...
				If MV_PAR37 == 1
			    	if TcSrvType() != "AS/400"
				       cQuery += "UNION "
						...
						cQuery += " AND NOTDEL.E2_EMISSAO   = '" + DTOS(dDataBase) +"'"
		...
		If MV_MULNATP .And. nOrdem == 2
			...
			#ENDIF
			If Empty(FwFilial("SE2")) .and. mv_par22 = 2
				Exit
			Endif
			Loop
		Endif  
...
//			Store 0 To nFil0,nFil1,nFil2,nFil3,nFil4,nFilTit,nFilJ
			While &cCond2 == cCarAnt .and. !Eof() .and. lContinua .and. SE2->E2_FILIAL == xFilial("SE2")
				...
				dbSelectArea("SA2")
				MSSeek(xFilial("SA2")+SE2->E2_FORNECE+SE2->E2_LOJA)             
				cFornece:=SE2->E2_NOMFOR
				cBanco:=POSICIONE("SA6",1,XFILIAL("SA6")+ALLTRIM(SA2->A2_BANCO),"A6_NOME")
				cAgencia:=SA2->A2_AGENCIA+" - "+SA2->A2_DVAGE
       		    cConta:= SA2->A2_NUMCON+" - "+SA2->A2_DVCTA
				IF SA2->A2_X_FGTO=="1" 
				 cFpgto:="Cheque" 
				elseif SA2->A2_X_FGTO=="2" 
				 cFpgto:="Ordem Pgto/Boleto"
				elseif SA2->A2_X_FGTO=="3" 
				 cFpgto:="Dinheiro/Caixa"
				elseif SA2->A2_X_FGTO=="4" 
				 cFpgto:="Deposito em Conta"
				ENDIF
				...
				If mv_par21 == 1
					nSaldo := SaldoTit(SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,SE2->E2_TIPO,SE2->E2_NATUREZ,"P",SE2->E2_FORNECE,mv_par15,dDataReaj,,SE2->E2_LOJA,,If(mv_par35==1,SE2->E2_TXMOEDA,Nil),IIF(mv_par34 == 2,3,1),.T.) // 1 = DT BAIXA    3 = DT DIGIT
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ				
				If MV_PAR22 == 1 .and. Empty(FwFilial("SE2"))
	            ...
	            cVencto  += iif(len(alltrim(cVencto))>0," - ","")+ dtoc(SE2->E2_VENCTO)
				#IFDEF TOP
					If TcSrvType() == "AS/400"
				...
				If dDataBase > SE2->E2_VENCREA 		//vencidos
					aDados[VL_NOMINAL] := nSaldo * If(SE2->E2_TIPO$MV_CPNEG+"/"+MVPAGANT, -1,1) //Alterado para demonstrar os descontos
					nJuros := 0
					dBaixa := dDataBase
				...
				EndIF
				If mv_par20 == 1	//1- Analitico  2-Sintetico
					aDados[HISTORICO] := alltrim(SE2->E2_HIST)+If(E2_TIPO $ MVPROVIS,"*"," ")
					#IFDEF TOP					
...
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin,010,"Desdobramento de duplicatas",oFont17N,20)
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin,010,"Forma de Pagamento:" ,oFont12N,20)
oReport:Say(nLin,500,cFpgto,oFont12N)
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin,010,"Depositar banco:" ,oFont12N,20)
oReport:Say(nLin,500,IIF(cFpgto=="Deposito em Conta",cBanco,""),oFont12N)
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin+1,010,"Agencia........:" ,oFont12N,20)
oReport:Say(nLin+1,500,IIF(cFpgto=="Deposito em Conta",cAgencia,""),oFont12N)
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin+1,010,"Conta corrente.:" ,oFont12N,20)
oReport:Say(nLin+1,500,IIF(cFpgto=="Deposito em Conta",cConta,""),oFont12N)
oReport:SkipLine(2)                                                        
nLin:=oReport:Row()
oReport:Say(nLin,010,"Vencimento(s) :" +substr(cVencto,1,62) ,oFont17N,20)
if len(cVencto)>62
  oReport:SkipLine(2)
  nLin:=oReport:Row()
  oReport:Say(nLin,010,substr(cVencto,63,len(cVencto)) ,oFont17N,20)
endif
oReport:SkipLine(2)
nLin:=oReport:Row()
oReport:Say(nLin,010,"Valor Liquido :" +transform(nTot2+nTot3,"@e 9,999,999.99"),oFont17N,20)
oReport:SkipLine(2)                                                        
nLin:=oReport:Row()
oReport:Say(nLin,010,"Fornecedor :" +substr(cFornece,1,62) ,oFont17N,20)
...
nLin:=oReport:Row()
oReport:Say( nLin ,1050,"V I S T O S",oFont17N)
oReport:SkipLine(2)
nLin:=oReport:Row() 
oReport:Box(nLIn,010,nLin+90,2380)
oReport:Box(nLIn,565,nLin+90,565)
oReport:Box(nLIn,1085,nLin+90,1085)
oReport:Box(nLIn,1685,nLin+90,1685)
oReport:Say( nLin+30 ,0020,"Setor Fiscal",oFont11N)
oReport:Say( nLin+30 ,0580,"Financeiro",oFont11N)               
oReport:Say( nLin+30 ,1100,"Solicitante",oFont11N)                  
oReport:Say( nLin+30 ,1700,"Controladoria",oFont11N) 
Return Nil
			...
			While &cCond2 == cCarAnt .and. !Eof() .and. lContinua .and. E2_FILIAL == xFilial("SE2")
				...
				If mv_par20 == 1				
					If !(Alltrim(cPaisLoc) $ "MEX|POR")
						@li,	0 PSAY SE2->E2_FORNECE+"-"+SE2->E2_LOJA+"-"+IIF(mv_par28 == 1, SubStr(SA2->A2_NREDUZ,1,21), SubStr(SA2->A2_NOME,1,21))																														
				...
				If dDataBase > SE2->E2_VENCREA 		//vencidos
					If mv_par20 == 1
						If aTamFor[1] >= 6
							@ li, Iif(cPaisloc $ "BRA|ARG",118,128) PSAY nSaldo  * If(SE2->E2_TIPO$MV_CPNEG+"/"+MVPAGANT, -1,1) Picture TM(nSaldo,15,nDecs)
						Else
				...
				EndIF
				If mv_par20 == 1
					@li,195 PSAY SUBSTR(SE2->E2_HIST,1,25)+ ;
...
IF li != 80
	If mv_par20 == 1
		li +=2
	Endif
	If (MV_MULNATP == .F. .Or. !Empty(FwFilial("SE2")))
		ImpTot150(nTot0,nTot1,nTot2,nTot3,nTot4,nTotTit,nTotJ,nDecs)
	Elseif MV_MULNATP .and. nOrdem != 2
		ImpTot150(nTot0,nTot1,nTot2,nTot3,nTot4,nTotTit,nTotJ,nDecs)	
	EndIf
	cbcont := 1
	roda(cbcont,cbtxt,"G")
...
if mv_par20 == 1
	@li,98 PSAY nTit0		 Picture TM(nTit0,14,nDecs)
endif  

If nOrdem == 2
	@li,112 PSAY nTit1		 Picture TM(nTit1,14,nDecs)  
	@li,132 PSAY nTit2		 Picture TM(nTit2,14,nDecs)  
	@li,148 PSAY nTit3		 Picture TM(nTit3,14,nDecs) 
else
...
if mv_par20 == 1
	@li,98 PSAY nFil0        Picture TM(nFil0,16,nDecs)
endif
@li,117 PSAY nFil1        Picture TM(nFil1,16,nDecs)
@li,132 PSAY nFil2        Picture TM(nFil2,16,nDecs)
