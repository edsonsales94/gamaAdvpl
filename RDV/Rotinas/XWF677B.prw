...
/*/{Protheus.doc} User Function XWF677B
    ( WorkFlow para Aprovação de Prestacao de contas )
    @type  User Function
    @author AOliveira
    @since 20/07/2021
    @version 1.0
...
IF nHdlLock < 0
	Conout("Rotina ( u_XWF677B() ) ja em execução.")
	Return(.T.)
EndIF
...
Local cAssunto		:= "Aprovação de Prestação de Contas Nr. " + XTRB->FLN_PRESTA
...
oWF:= TWFProcess():New("WF677B", "Aprovação de Prestação de Contas")
...
If FLN->(DbSeek( cNum))
	...
	if FLF->(DbSeek( FLN->(FLN_FILIAL+FLN_TIPO+FLN_PRESTA+FLN_PARTIC) ))
		...
		if !(FLF->FLF_STATUS <> '1')
			...
			If FLN->(dbSeek( cNum ))
				If FLN->FLN_STATUS == '1'
					//"A operação de aprovação não foi concluída."
					conout('XWF677B3 -> RDV  operação de aprovação não foi concluída. -> '+CValToChar(cNum))
					lRet := .F.
					lOK := .F.
				else
